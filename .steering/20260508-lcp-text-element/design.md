...
- **見出し文言を変更**: 「LGTM 画像一覧」(機能名) →「LGTM 画像でレビューを楽しく」
  (バリュープロポジション)。PRD の「コードレビュー文化をより楽しいものにする」と
  整合し、ヒーローらしい温度感に合わせる
...
`getByRole('heading', { name: 'LGTM 画像一覧' })` を `'LGTM 画像でレビューを楽しく'` に
...
| 要素 | 概算 bbox |
|------|----------|
| ヒーロー h1 (text-6xl, ~60px, 単行 leading-tight) | ~750 × 75 = **56,250** |
| ヒーロー段落 (text-lg, ~18px × leading 1.625 = 29px, 3 行 ラップ) | 672 × 87 = **58,464** |
| 1 枚目画像 (260px 幅, 4:3) | 260 × 195 = **50,700** |

→ h1 / p ともに画像より大きく、いずれが LCP に選ばれてもテキスト LCP となる。
`md:text-6xl` (60px) で h1 単行 bbox ~56k > 画像 50.7k のマージンを確保する。

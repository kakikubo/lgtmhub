...
- 投稿者ブロックがリンク (`<a>`) のときのみ、ホバー時に表示名にアンダーラインを表示する（fallback `<div>` ではアンダーラインを表示しない）。実装は `<a>` 側に `group`、表示名 `<span>` 側に `group-hover:underline` を付与する。
- アバターには装飾を加えない。
- フォーカスリングは `<a>` に明示的に `focus:outline-none focus:ring-2 focus:ring-gray-900` を付与し、既存の画像リンクと統一する。

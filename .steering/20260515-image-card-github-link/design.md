...
- 表示名 `<span>` に `hover:underline` を追加する。アバターには装飾を追加しない。
- `<a>` の Tailwind クラスは `flex items-center gap-2` をそのまま継承し、`focus:outline-none focus:ring-2 focus:ring-gray-900 rounded` を付与（既存の画像リンクと同等のフォーカスリング）。
...
const uploaderContent = (
  <>
    ...
    <span className="text-sm text-gray-700 truncate hover:underline">

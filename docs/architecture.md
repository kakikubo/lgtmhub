...
- **初期画像一覧のキャッシュ**: `src/lib/cache/list-home-images.ts` の `getHomeImagesInitial` を `'use cache'` ディレクティブ化し、`cacheTag('lgtm-images:list')` でタグ付け・`cacheLife('max')` で寿命を最長化。無効化は `revalidateTag('lgtm-images:list', 'max')`（第2引数のプロファイル必須）に委ねる。`'use cache'` 配下では `cookies()` を呼べないため、Cookie 非依存の `createAnonClient` を採用し、RLS の `"anyone can view active images"` ポリシーで匿名 SELECT を担保する
...
- **proxy 化**: Next.js 16 で `middleware.ts` は `proxy.ts` にリネーム（edge runtime 非対応）。本プロジェクトの `@supabase/ssr` `createServerClient` は Node.js runtime デフォルトで影響なし

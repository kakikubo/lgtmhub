...
```
GET /api/images/search?q={keyword}&page={n}
```

**認証**: 必須（Supabaseセッション。`/images/new` の検索タブからのみ呼ばれる前提）

**用途**: 画像登録ページのキーワード検索タブで、外部画像プロバイダーから候補画像を取得する。MVP のプロバイダーは Pexels。

**クエリパラメータ**:
| パラメータ | 型 | デフォルト | 説明 |
|-----------|----|-----------|----|
| `q` | string | - | 検索キーワード。1〜100 文字。前後空白は trim |
| `page` | number | 1 | ページ番号（最大 50） |
...
`Cache-Control: public, s-maxage=60, stale-while-revalidate=300` を付与し Vercel Edge でキャッシュする（プロバイダーのレート上限緩和のため）。

**エラーレスポンス**:
- 400 Bad Request: クエリパラメータ不正
- 401 Unauthorized: 未ログイン
- 502 Bad Gateway: 外部プロバイダーが正常応答しなかった
- 503 Service Unavailable: 外部プロバイダーのレート上限到達 / API キー未設定（運用ミス）

選択した `imageUrl` はクライアントから既存 `POST /api/images` に送信され、SSRF 検証 / 重複検知 / LGTM 合成 / Vercel Blob 保存の既存パイプラインを通る。
...
検索タブで選択した画像 URL は既存の `POST /api/images` に送られ、URL 入力タブと同じ合成・重複検知・登録パイプラインを通る。

...
5. LGTM文字合成 + WebP変換 + 266×199 に中央クロップ
...
sequenceDiagram
    ...
    API->>Sharp: LGTM文字合成 + WebP変換 + 266×199 中央クロップ
...
```typescript
const TARGET_WIDTH = 266;
const TARGET_HEIGHT = 199;

async function composeLgtmImage(imageBuffer: Buffer): Promise<Buffer> {
  // SVGでLGTM文字を生成（白文字+黒縁）
  const fontSize = Math.floor(TARGET_WIDTH * 0.15);
  const svgText = `
  ...
  // 物理サイズを 266×199 に固定し、アスペクト比違いの元画像は中央クロップする。
  // 元画像が 266×199 より小さい場合は拡大して揃える（withoutEnlargement は付けない）。
...
- `composeLgtmImage()`: 合成画像のメタデータ検証（WebP形式、266×199 固定、中央クロップ動作）

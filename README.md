...
4. `npm run db:stop && npm run db:start` で再起動(`supabase/config.toml` の `[auth.external.github]` がこれらの env を参照する)。起動ログに `WARN: environment variable is unset: GITHUB_OAUTH_*` が出ないことを確認。
